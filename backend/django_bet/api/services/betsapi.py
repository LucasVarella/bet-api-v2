        ...
        while True:
            ...
            for match in result['response']:
                ...
                for market in match['bookmakers'][0]['bets']:
                    ...
                    try:
                        # O Market existe, atualize-o
                        market_obj = Market.objects.get(number=market['id'], match=match_obj)
                        
                        # Atualiza as Bets existentes
                        bets_to_update = []
                        bets_to_create = []
                        ...
                        # O Market não existe, crie-o
                        uu_id = uuid.uuid4()
                        market_obj = Market(number=market['id'], match=match_obj, id= uu_id, name= market["name"])
                        market_obj.save()
...
def execute_odds_football():
    print('Atualizando odds')
    start_time = time.time()
    fetch_odds_football(8,3)
    end_time = time.time()
    print(f"Tempo final de execução: {end_time - start_time:.2f} segundos\n")